...
function AppointmentsPage() {
  ...
  // useEffect para cargar las citas solo si se tiene la contraseña correcta
  useEffect(() => {
    ...
    fetchAppointments();
  }, [isAuthorized]); // El efecto se ejecuta cuando isAuthorized cambia
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-100">
        <form onSubmit={handlePasswordSubmit} className="bg-white p-8 rounded-lg shadow-lg">
          ...
          <p className="mb-4 text-gray-600 text-center">Introduce la contraseña para ver las citas.</p>
          <input
            type="password"
          ...
          {error && <p className="mt-4 text-red-500 text-sm italic text-center">{error}</p>}
        </form>
      </div>
    );
  }
  ...
  if (loading) {
    return <div className="text-center mt-8">Cargando citas...</div>;
  }
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {appointments.length === 0 ? (
        <p className="text-center text-gray-500">Aún no hay citas agendadas.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {appointments.map((appointment) => (
            <div key={appointment._id} className="bg-white p-6 rounded-lg shadow-md border-t-4 border-blue-500">
